 ...
 */
public class MyMapLayerTable extends MapLayerTable {
     ...
     */
    public MyMapLayerTable(JMapPane pane) {
        init();
        setMapPane(pane);
    }
    ...
    private void onLayerItemClicked(MouseEvent ev, boolean doubleClick) {
        ...
        if (item >= 0) {
            ...
            if (r.contains(ev.getPoint())) {
                ...
                if (ev.getButton() == 3) {
                	doShowDetailPopup(layer, ev.getPoint().x, ev.getPoint().y);
    ...
    private void doShowDetailPopup(final MapLayer layer, int x, int y) {
    	...
    	details.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
		    	try {
					new FeatureDetailFrame(pane, layer);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
